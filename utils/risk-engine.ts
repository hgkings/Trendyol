...
export function calculateRisk(input: ProductInput, result: CalculationResult): RiskResult {
  ...
  // Margin score (0–40)
  let marginScore = 0;
  if (result.margin_pct >= 20) {
    marginScore = 40;
  } else if (result.margin_pct >= 10) {
    marginScore = 20 + ((result.margin_pct - 10) / 10) * 20;
  } else if (result.margin_pct >= 0) {
    marginScore = (result.margin_pct / 10) * 20;
  }
  if (result.margin_pct < 10) {
    factors.push({
      name: 'Düşük Kar Marjı',
      impact: Math.round(40 - marginScore),
      description: `Kar marjı %${result.margin_pct.toFixed(1)} — ${result.margin_pct < 0 ? 'Zarar durumu' : 'Riskli seviye'}`,
    });
  }

  // Return score (0–20)
  let returnScore = 0;
  if (input.return_rate_pct <= 5) {
    returnScore = 20;
  } else if (input.return_rate_pct <= 15) {
    returnScore = 20 - ((input.return_rate_pct - 5) / 10) * 10;
  } else {
    returnScore = Math.max(0, 10 - ((input.return_rate_pct - 15) / 10) * 10);
  }
  if (input.return_rate_pct > 10) {
    factors.push({
      name: 'Yüksek İade Oranı',
      impact: Math.round(20 - returnScore),
      description: `İade oranı %${input.return_rate_pct} — Ortalamanın üzerinde iade maliyeti.`,
    });
  }

  // Ad dependency (0–20)
  const ad_ratio = input.sale_price > 0 ? (input.ad_cost_per_sale / input.sale_price) * 100 : 0;
  let adScore = 0;
  if (ad_ratio <= 3) {
    adScore = 20;
  } else if (ad_ratio <= 10) {
  ...
  if (input.commission_pct <= 12) {
    commissionScore = 20;
  } else if (input.commission_pct <= 20) {
    commissionScore = 20 - ((input.commission_pct - 12) / 8) * 10;
  } else {
    commissionScore = Math.max(0, 10 - ((input.commission_pct - 20) / 10) * 10);
  }
  if (input.commission_pct > 20) {
    factors.push({
      name: 'Yüksek Komisyon',
      impact: Math.round(20 - commissionScore),
      description: `Komisyon oranı %${input.commission_pct} — Pazaryeri kesintileri kârı eritiyor.`,
    });
  }
