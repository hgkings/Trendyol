          ...
          <tbody className="divide-y divide-border/20">
            {rows.map((row) => (
              <tr key={row.scenario} className="transition-colors hover:bg-muted/30">
                ...
                <td className={`py-2.5 text-right font-medium ${row.newProfit >= 0 ? 'text-emerald-700 dark:text-emerald-400' : 'text-red-400'}`}>
                  {formatCurrency(row.newProfit)}
                </td>
                <td className={`py-2.5 text-right ${row.difference >= 0 ? 'text-emerald-700 dark:text-emerald-400' : 'text-red-400'}`}>
                  {row.difference >= 0 ? '+' : ''}{formatCurrency(row.difference)}
                </td>
                <td className={`py-2.5 text-right ${row.percentChange >= 0 ? 'text-emerald-700 dark:text-emerald-400' : 'text-red-400'}`}>
